...
interface Metrics {
  from_date: string;
  to_date: string;
  vpn_filtering_active: boolean;
  ipqs_active: boolean;
  traffic_trust_private_ip?: boolean;
  unique_sessions_valid: number;
  server_clicks_valid?: number;
  page_views_valid: number;
  view_content_valid: number;
  add_to_cart_valid: number;
  initiate_checkout_valid: number;
  orders_valid: number;
  revenue_valid: number;
  conversion_rate: number;
  aov_valid: number;
  upsell_orders_valid: number;
  upsell_rate_valid: number;
  cta_clicks_valid?: number;
  daily: DailyRow[];
}
...
export default function AdminDashboardPage() {
  ...
  const [tab, setTab] = useState<"overview" | "orders" | "system">("overview");
    ...
    async (t: string) => {
      ...
      try {
        const [mRes, oRes] = await Promise.all([
          fetch(`/admin/metrics?${q}`, { headers: authHeaders(t) }),
          fetch(`/admin/orders?${ordQ}`, { headers: authHeaders(t) }),
        ]);
        ...
        if (!mRes.ok || !oRes.ok) throw new Error("fetch failed");
        setMetrics(await mRes.json());
        setOrders(await oRes.json());
      } catch {
        setError("Could not load dashboard. Check API and date range.");
      }
      setLoading(false);
    },
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        let detail =
          res.status === 503 ? "Admin login not configured on server." : "Invalid credentials.";
        try {
  ...
  if (!token) {
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4" dir="ltr">
        ...
        >
          <div>
            <h1 className="text-2xl font-bold text-white tracking-tight">{BRAND.nameEn} — Admin</h1>
            <p className="text-slate-400 text-sm mt-1">DTC analytics & orders (KSA-valid traffic)</p>
          </div>
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex" dir="ltr">
      <aside className="w-56 border-r border-slate-800 bg-slate-900/80 shrink-0 hidden md:flex flex-col">
        <div className="p-5 border-b border-slate-800">
          ...
          <p className="text-xs text-slate-500 mt-0.5">{BRAND.domain.replace("https://", "")}</p>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-14 border-b border-slate-800 flex items-center px-4 gap-3 bg-slate-900/50">
          <div className="flex md:hidden gap-1">
            {(["overview", "orders", "system"] as const).map((id) => (
              <button
                key={id}
                onClick={() => setTab(id)}
            ...
            <div className="space-y-6">
              <div className="flex flex-wrap gap-2">
                {metrics.traffic_trust_private_ip && (
                  <span className="text-xs px-2 py-1 rounded-full border border-rose-500/50 text-rose-300 bg-rose-950/40">
                    TRAFFIC_TRUST_PRIVATE_IP: on (dev only — private IPs count as valid)
                  </span>
                )}
                <span
                ...
                >
                  MaxMind VPN / DC filter: {metrics.vpn_filtering_active ? "on" : "off (country-only fallback)"}
                </span>
                <span
                  className={`text-xs px-2 py-1 rounded-full border ${
                    metrics.ipqs_active
                ...
                >
                  IPQualityScore: {metrics.ipqs_active ? "on" : "optional"}
                </span>
              </div>
              <p className="text-slate-400 text-sm max-w-3xl">
                All funnel numbers below are <strong className="text-white">KSA-valid, non-VPN / non-datacenter</strong>{" "}
                when MaxMind credentials are set. Orders and sessions must match <code className="text-amber-400/90">traffic_valid = true</code>.
              </p>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                {[
                  ...
                  ["CTA clicks (tracked)", (metrics.cta_clicks_valid ?? 0).toLocaleString()],
              ...
              <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-5">
                <h2 className="text-lg font-semibold text-white mb-4">Daily revenue (valid traffic orders)</h2>
              ...
              <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-5">
                <h2 className="text-lg font-semibold text-white mb-4">Funnel (valid sessions)</h2>
                <div className="grid gap-2 text-sm">
                  {[
                    ...
                    const prev = i === 0 ? (n as number) : (arr[i - 1][1] as number);
                    const pct = prev > 0 ? (((n as number) / prev) * 100).toFixed(0) : "—";
                    return (
                      <div
                        key={label as string}
            ...
            <div className="space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-end gap-3 flex-wrap">
                <div>
                  ...
                  <p className="text-slate-500 text-xs mt-0.5">
                    Filtered by overview date range · KSA-valid flag from MaxMind + IPQS on the server
                  </p>
                </div>
              ...
              <div className="rounded-2xl border border-slate-800 overflow-hidden bg-slate-900/40">
                <div className="overflow-x-auto">
                    ...
                    <tbody>
                      {orders.map((o) => (
                        ...
                        >
                          ...
                          <td className="px-4 py-3">
                            {o.traffic_valid === true && (
                              <span className="text-emerald-400 text-xs font-bold">YES</span>
                            )}
                            {o.traffic_valid === false && (
                              <span className="text-rose-400 text-xs font-bold">NO</span>
                            )}
                  ...
                  {orders.length === 0 && (
                    <p className="text-center text-slate-500 py-12">No orders in database.</p>
                  )}
                </div>
              </div>
            </div>
          ...
          {tab === "system" && (
            <div className="max-w-xl space-y-4">
              ...
              <p className="text-slate-400 text-sm">
                Deletes <strong className="text-rose-300">all</strong> orders, line items, and tracking events. Requires
                admin JWT.
              </p>
              <button
                type="button"
                ...
                onClick={async () => {
                  ...
                  setClearBusy(true);
                  try {
                    const res = await fetch("/admin/orders/clear", {
                      method: "DELETE",
            ...
            <div className="px-6 py-5 space-y-5">
              <div className="rounded-2xl border border-slate-800 bg-slate-900/50 p-4 space-y-3">
                ...
                {selectedOrder.session_id ? (
                  <p className="text-slate-500 text-[10px] font-mono break-all">
                    session: {selectedOrder.session_id}
                  </p>
                ) : null}
              </div>
              ...
              {selectedOrder.upsell_accepted && (
                <div className="rounded-2xl border border-emerald-500/35 bg-emerald-950/25 px-4 py-3 text-sm text-emerald-200 flex items-center gap-2">
                  ...
                  <span>
                    Upsell accepted
                    {selectedOrder.upsell_sku ? <span className="font-mono text-emerald-300"> · {selectedOrder.upsell_sku}</span> : ""}
                  </span>
                </div>
              )}
              ...
              <div className="flex justify-between items-end gap-4 rounded-2xl bg-gradient-to-r from-slate-900 to-slate-950 border border-slate-800 px-5 py-4">
                <div>
                  ...
                  <p className="text-amber-400/80 text-[10px] font-mono mt-1 max-w-[200px] truncate" title={selectedOrder.client_ip || ""}>
